...
/**
 * Represents a board manager for objectives, teams, and scores for a group of players.
 * This class facilitates the creation, modification, and removal of board elements.
...
public class Board {
     ...
     *
     * @param player The player for whom to retrieve the associated boards.
     * @return An immutable set containing the associated boards, or null if no boards are found
     *         for the specified player.
     */
    public static Set<Board> getPlayerBoards(Player player) {
        Set<Board> boards = PLAYER_BOARDS.get(player.getUniqueId());
        return (boards != null) ? ImmutableSet.copyOf(boards) : null;
    }

    /**
     * Remove all the associated boards with the specified player.
     * @param player The player for whom to remove the associated boards.
     */
    public static void removePlayerBoards(Player player) {
        Set<Board> boards = PLAYER_BOARDS.remove(player.getUniqueId());
        if (boards != null) {
            boards.forEach(board -> board.removePlayer(player));
        }
    }

    private static void setPlayerBoard(Player player, Board board) {
        PLAYER_BOARDS.computeIfAbsent(player.getUniqueId(), uuid -> new HashSet<>())
                .add(board);
    }

    private static void removePlayerBoard(Player player, Board board) {

        Set<Board> boards = PLAYER_BOARDS.get(player.getUniqueId());

        if (boards != null) {
            boards.remove(board);
            // Remove the mapping if the set becomes empty
            if (boards.isEmpty()) {
    ...
    /**
     * Adds all the given players to the board.
     * @param players A varargs array of players to add to the board.
     */
    public void addAll(Player... players) {
        addAll(Arrays.asList(players));
    }

    /**
     * Adds all the given players to the board.
     * @param players A {@code Collection} of players to add to the board.
     */
    ...
    /**
     * Adds a player to the board, initializing the board elements.
     *
     * @param player The player to be added to the board.
     * @throws IllegalArgumentException If the player is null, or if it has already been added to the board.
     */
    ...
    /**
     * Removes a player from the board, clearing the associated elements.
     *
     * @param player The player to be removed from the board.
     * @throws IllegalArgumentException If the player is null, or if it has not been added to the board.
     */
    public void removePlayer(Player player) {
        ...
        teamsByName.values().forEach(
                team -> {
                    Set<String> entities = !(team.getEntities().isEmpty()) ? team.getEntities() : null;
                    packets.add(createPacketTeam(team, PacketTeam.Mode.REMOVE, entities));
                }
        );
    ...
    /**
     * Retrieves an immutable set of unique player UUIDs associated with this board.
    ...
    /**
     * Retrieves an immutable set of Objective instances associated with this board.
    ...
    /**
     * Retrieves an immutable set of Team instances associated with this board.
